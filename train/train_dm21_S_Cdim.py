...
class QuantumDeepField(nn.Module):
    def __init__(self, device, N_orbitals,
        ...
        nn.init.normal_(self.zeta.weight, mean=1, std=2)
        self.G_max = 840
        self.N_cut_max = 130
        self.N_el_max = 68
...
#        coefficients = torch.cat(coefficients)
        ...
        coeffi_batch = self.ceff_encoder(coeffi_input)#batchsize* N_cut_max *N_el_max 
        ...
        for i, basis_matrix in enumerate(basis_matrix_lst):
            ...
            coeffi_matrix = coeffi_batch[i,:basis_matrix_t.shape[0]-int(N_electrons_lst[i]), :int(N_electrons_lst[i])].t()#Note #batchsize* N_c-N_e *N_e 
            #NOTE AUTOMATIC OTHOGNALIZATION
            psi, psi_lap, psi_gra, psi_cpl, psi_cpl_lap = get_orbitals_triple(coeffi_matrix.transpose(0,1), basis_matrix_t, l_basis_matrix_t, g_basis_matrix_t, self.grid_interval)
