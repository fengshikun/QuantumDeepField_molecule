...
def gram_schmidt_triple(vv, grid_interval):
    
    nk = vv.size(0) #Ne
    # ng= vv.size(1) #G
    rank=np.linalg.matrix_rank(vv.cpu().detach().numpy())#++
    if rank < nk:
        print("The input is not full-rank")
...
psi: 
"""
def get_orbital(coeff, phi_gto, grid_interval):
    ...
    psi = gram_schmidt_triple(psi_zero,  grid_interval)
    return psi
...
if __name__ == "__main__":
    ...
    psi=get_orbital(coeff, phi_gto, grid_interval)
    print(np.linalg.matrix_rank(psi.cpu().detach().numpy()),Ne)
    print(torch.matmul(psi, torch.transpose(psi, 0, 1)) * grid_interval**3)
'''
import numpy as np
