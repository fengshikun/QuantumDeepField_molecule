...
class QuantumDeepField(nn.Module):
    ...
    def LCAO2(self, inputs):
        ...
        """LCAO."""
        basis_matrix_all, l_basis_matrix_all = self.basis_matrix(atomic_orbitals,
    ...
    def forward(self, data, train=False, target=None, predict=False):
        ...
        if predict:  # For demo.
            with torch.no_grad():
                ...
                V_n_lst = data[7]
                psi_lst, psi_lap_lst, psi_cpl_lst, psi_cpl_lap_lst = self.LCAO2(inputs)
                ...
                for i, psi in enumerate(psi_lst):
                    ...
                    # way3 psi, psi_lap --> K
                    K = -0.5 * torch.matmul(psi, psi_lap.transpose(0, 1))
                    # way4 psi_cpl, psi_cpl_lap --> K_cpl
                    K_cpl = -0.5 * torch.matmul(psi_cpl, psi_cpl_lap.transpose(0, 1))
                    K_lst.append(K)
                    # K_lst[i] = K
                    # K_cpl_lst[i] = K_cpl
                ...
                for i in range(batch_num):
                    ...
                    E_n[i] = torch.sum(torch.from_numpy(V_n_lst[i]).cuda() * densities[d_n:d_n+d_ni]) 
                    V_xcH_lst.append(V_xcH[d_n:d_n + d_ni])
                    E_xch[i] = -torch.sum(V_xcH[d_n:d_n + d_ni] * densities[d_n:d_n + d_ni])
                    d_n += d_ni
