...
def create_dataset(dir_dataset, filename, basis_set,
                   radius, grid_interval, orbital_dict, dataset_length, property=True):
    ...
    # """Load a dataset."""
    # dir_preprocess = '/data/liuao/deep-DFT/QuantumDeepField_molecule-main/train2/'
    # with open(dir_preprocess + filename + '.txt', 'r') as f:
    #     dataset = f.read().strip().split('\n\n')
    coefs = np.load('/data3/yuyan/ml-dft/water_102/dft_densities.npy')
    atom_xyzs =  np.load('/data3/yuyan/ml-dft/water_102/structures.npy')
    N = atom_xyzs.shape[0] #102
    percent = 10
    ...
    for n in range(N):
        ...
        for ind in range(atom_xyzs.shape[1]):
            x, y, z = atom_xyz[ind][0],atom_xyz[ind][1],atom_xyz[ind][2]              
            atomic_number = [1,1,8]
            ...
            elif atomic_number >= 11 and atomic_number <= 18:
                aqs = ([(atom+'1s' + str(i), 1) for i in range(inner)] +
                       [(atom+'2s' + str(i), 2) for i in range(inner)] +
                       [(atom+'2p' + str(i), 2) for i in range(inner)]+
                       [(atom+'3s' + str(i), 3) for i in range(outer)] +
                       [(atom+'3p' + str(i), 3) for i in range(outer)]                    
                       )
            for a, q in aqs:
...
#        sys.exit()
        ...
        # data += [field_coords.astype(np.float32)]
        coefficient=coef[ind]
        densities = #TODO inverse fourier (fft)
        data += [0,0,0,densities]
...
if __name__ == "__main__":
    ...
    parser.add_argument('dataset_length') #NOTE new arg
    ...
    create_dataset(dir_dataset, 'water_102',
                   basis_set, radius, grid_interval, orbital_dict, dataset_length)
    print('-'*50)
